...
def filter_static_frames_indices(
    labels_data: dict,
    enabled: bool = True,
    ...
    """
    Filter out static frames and return the indices of dynamic frames.
    
    Args:
        labels_data: Dictionary of labels loaded from labels.json
        enabled: Whether to filter static frames
        position_threshold: Threshold for position change (meters)
        rotation_threshold: Threshold for rotation change (radians)
        gripper_threshold: Threshold for gripper change (0-1)
        
    Returns:
        List of frame indices to keep
    """
    if (not enabled) or len(labels_data) <= 1:
        return list(range(len(labels_data)))
    
    selected_frames_indices = []
    prev_pos = labels_data[str(0)]["xyz"]
    prev_quat = np.array(labels_data[str(0)]["quats"]) / np.linalg.norm(labels_data[str(0)]["quats"])
    prev_gripper = labels_data[str(0)]["gripper"]
    selected_frames_indices.append(0)
    
    for i in range(1, len(labels_data)):
        # Extract position (translation) and rotation (quaternion)
        curr_pos = labels_data[str(i)]["xyz"]
        curr_quat = np.array(labels_data[str(i)]["quats"]) / np.linalg.norm(labels_data[str(i)]["quats"])
        curr_gripper = labels_data[str(i)]["gripper"]
        
        # Calculate position difference
        pos_diff = np.linalg.norm(np.array(curr_pos) - np.array(prev_pos))
        gripper_diff = np.abs(curr_gripper - prev_gripper)
        
        # Calculate rotation difference using quaternion distance
        dot_product = np.abs(np.dot(prev_quat, curr_quat))
        dot_product = np.clip(dot_product, 0, 1)  # Ensure valid range
        rot_diff = 2 * np.arccos(dot_product)
        
        # If both position and rotation changes are below thresholds, mark as static
        if pos_diff < position_threshold and rot_diff < rotation_threshold and gripper_diff < gripper_threshold:
            continue
            
        prev_pos = curr_pos
        prev_quat = curr_quat
        prev_gripper = curr_gripper
        selected_frames_indices.append(i)
    
    return selected_frames_indices
...
class DataLoaderConfig:
    ...
    goal_pose_dim:      The number of dimensions to use for the goal pose.
    filter_static_frames: Whether to filter out static frames from trajectories.
    static_filter_position_threshold: Position threshold for static frame filtering (meters).
    static_filter_rotation_threshold: Rotation threshold for static frame filtering (radians).
    static_filter_gripper_threshold: Gripper threshold for static frame filtering (0-1).
    """
    ...
    def __init__(
        ...
        goal_pose_dim: int = 0,
        filter_static_frames: bool = False,
        static_filter_position_threshold: float = 0.003,
        static_filter_rotation_threshold: float = 0.1,
        static_filter_gripper_threshold: float = 0.05,
    ):
        ...
        # Static frame filtering parameters
        self.filter_static_frames = filter_static_frames
        self.static_filter_position_threshold = static_filter_position_threshold
        self.static_filter_rotation_threshold = static_filter_rotation_threshold
        self.static_filter_gripper_threshold = static_filter_gripper_threshold
        ...
        self.use_object_pos = goal_pose_dim > 0
        self.object_pose_dim = goal_pose_dim
    ...
    def _build_trajectories(self):
        ...
        for root_path in tqdm.tqdm(
            ...
            if not self._validate_poses(poses):
                continue
            # Apply static frame filtering if enabled to get the effective trajectory length
            filtered_indices = filter_static_frames_indices(
                poses,
                enabled=self.filter_static_frames,
                position_threshold=self.static_filter_position_threshold,
                rotation_threshold=self.static_filter_rotation_threshold,
                gripper_threshold=self.static_filter_gripper_threshold,
            )
            trajectory_length = len(filtered_indices)
