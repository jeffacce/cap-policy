...
from dataloaders.object_pos_data import ObjectPosLoader
...
class AbstractVideoDataset(ABC, Dataset):
    ...
    def _get_object_pos_reader(self, index: int) -> ObjectPosLoader:
        if index not in self._object_pos_reader_cache:
            self._object_pos_reader_cache[index] = ObjectPosLoader(
                object_pose_dim=self._data_config.object_pose_dim,
                pos_labels_path=self._data_config.trajectories[index],
                
            )
        return self._object_pos_reader_cache[index]
