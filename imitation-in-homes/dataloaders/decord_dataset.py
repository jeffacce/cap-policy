...
from dataloaders.utils import DataLoaderConfig, TrajectorySlice, flatten_nested_lists, filter_static_frames_indices
...
class DecordDataset(AbstractVideoDataset):
    ...
    def __init__(
        ...
        self._prefetch_to_ram = prefetch_to_ram
        self._filesystem_path_to_ram_path = {}
        self._include_trajectory_end = config.include_trajectory_end_flag

        # Cache for filtered frame indices per trajectory
        self._filtered_frame_indices_cache = {}
        ...
        if self._prefetch_to_ram:
            self.videos = []
            print("Prefetching videos to RAM...")
            for i in tqdm.tqdm(range(self._num_videos)):
                reader = self._get_video_reader(i)
                reader.seek(0)
                vid = reader.get_batch(np.arange(len(reader)))
                vid = einops.rearrange(vid, "... h w c -> ... c h w")
    ...
    def _get_object_pos(self, trajectory_slice: TrajectorySlice) -> torch.Tensor:
        frame_indices = self._convert_subslice_to_video_indices(trajectory_slice)
        video_index = trajectory_slice.trajectory_index
        object_pos_reader = self._get_object_pos_reader(video_index)
        object_pos = object_pos_reader.get_batch(frame_indices)
        return object_pos
    ...
    def _get_video_frames(self, trajectory_slice: TrajectorySlice) -> torch.Tensor:
        ...
        video_index = trajectory_slice.trajectory_index
        if self._prefetch_to_ram:
            frames = self.videos[video_index][frame_indices]
        else:
    ...
    def _convert_subslice_to_indices(
        ...
        # If filtering is enabled, map these filtered indices to original video frame indices
        filtered_indices = self._get_filtered_frame_indices(trajectory_slice.trajectory_index)  # [5, 8, 12, ...] original indices
        return filtered_indices[frame_indices]
    
    def _get_filtered_frame_indices(self, video_index: int) -> Union[None, np.ndarray]:
        """Get the filtered frame indices for a video trajectory."""
        if video_index not in self._filtered_frame_indices_cache:
            # Load the labels for this trajectory
            labels_path = self._data_config.trajectories[video_index] / "labels.json"
            with open(labels_path, "r") as f:
                labels_data = json.load(f)
            
            # Get filtered indices using the same function as PoseDataLoader
            filtered_indices = filter_static_frames_indices(
                labels_data,
                enabled=self._data_config.filter_static_frames,
                position_threshold=self._data_config.static_filter_position_threshold,
                rotation_threshold=self._data_config.static_filter_rotation_threshold,
                gripper_threshold=self._data_config.static_filter_gripper_threshold,
            )
            self._filtered_frame_indices_cache[video_index] = np.array(filtered_indices)
        
        return self._filtered_frame_indices_cache[video_index]
