...
class Workspace:
    ...
    def run(self):
        for epoch in (
            ...
            self._epoch = epoch
            self._train_epoch()
            if self._cfg.sim_eval and epoch % self._cfg.sim_eval_every == 0:
                self._sim_eval_epoch()
    ...
    @torch.no_grad()
    def _sim_eval_epoch(self):
        self.model.eval()
        self.loss_fn.eval()
        if accelerator.is_local_main_process:
            condition = None
            if self._cfg["goal_dim"] == 2:
                condition = "2d"
            elif self._cfg["goal_dim"] == 3:
                condition = "3d"
            results = eval_sim(
                model=self.model,
                loss_fn=self.loss_fn,
                buffer_size=self._cfg["obs_window_size"],
                objects_set_path=self._cfg["sim_objects_set_path"],
                num_envs=self._cfg["sim_num_envs"],
                num_episodes=self._cfg["sim_num_episodes"],
                num_episodes=self._cfg["sim_seed"],
                render=False,
                condition=condition,
            )
            ...
            accelerator.log({
                "sim_eval/success_rate": success_rate,
                "_epoch": self._epoch,
            })
