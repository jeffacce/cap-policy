...
from utils import AveragingDict, set_seed_everywhere, get_demos, Callbacks
...
class DirectIndexDataloader:
    def __init__(self, dataset, batch_size, shuffle, drop_last, preindex=True):
    ...
    def __iter__(self):
        ...
        if self.preindex:
            shuffled_data = self.dataset[indices].contiguous()
            for i in range(num_batches):
                yield (torch.zeros(1), shuffled_data[i * self.batch_size : (i+1) * self.batch_size])
        else:
            for i in range(num_batches):
                idx = indices[i * self.batch_size : (i + 1) * self.batch_size]
                yield (torch.zeros(1), self.dataset[idx])
...
class Workspace:
    ...
    def _setup_dataloaders(
        ...
        all_actions = [train_dataset[i][1] for i in tqdm.trange(len(train_dataset), desc="Loading actions")]
        all_actions = torch.from_numpy(np.concatenate(all_actions))
        all_actions = all_actions.unsqueeze(1).cuda()
        train_dataloader = DirectIndexDataloader(all_actions, self._cfg.batch_size, shuffle=True, drop_last=False)
    ...
    def _train_epoch(self):
        ...
        iterator = tqdm.tqdm(
            self._train_dataloaders, disable=True
        )
        self._total_loss = 0
